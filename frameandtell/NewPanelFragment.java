...
//				imm.hideSoftInputFromWindow(panelName.getWindowToken(), 0);
		...
		saveButton.setOnClickListener(new View.OnClickListener() {
			...
			//	panel.setRating(mealRating.getSelectedItem().toString());
				...
				panel.saveInBackground(new SaveCallback() {
		...
		panelPreview = (ParseImageView) v.findViewById(R.id.panel_preview_image);
		panelPreview.setVisibility(View.INVISIBLE);
	...
	public void startCamera() {
		...
		transaction.addToBackStack("NewPanelFragment");
		transaction.commit();
	}
	...
	public void onResume() {
		super.onResume();
		ParseFile photoFile = ((EditPanelActivity) getActivity())
				.getCurrentPanel().getPhotoFile();
		if (photoFile != null) {
			panelPreview.setParseFile(photoFile);
			panelPreview.loadInBackground(new GetDataCallback() {
				@Override
				public void done(byte[] data, ParseException e) {
					panelPreview.setVisibility(View.VISIBLE);
				}
			});
		}
	}
